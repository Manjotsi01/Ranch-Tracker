...
async function migrate() {
  ...
  const collection = mongoose.connection.collection('animals')
  ...
  if (missingType > 0) {
    const r1 = await collection.updateMany(
      { $or: [{ type: { $exists: false } }, { type: null }, { type: '' }] },
      { $set: { type: 'COW' } }  // ← change to 'BUFFALO' if most are buffaloes
    )
  ...
  const noTagNo = await collection.updateMany(
    { $and: [
  ...
  if (other > 0) console.log(`   ⚠ Other:   ${other} (check these manually)`)
